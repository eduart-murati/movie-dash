...
const MovieStarRating = ({rating}: Props) => {
    ...
    for (let i = 1; i <= numStars; i++) {
        if (i <= normalizedRating) {
            ...
            // Half star
            stars.push(<Icon as={FaStarHalfAlt} color="gold" boxSize={starSize} key={i} />);
        } else {
            // Empty star
            stars.push(<Icon as={FaStar} color="gray.300" boxSize={starSize} key={i} />);
        }
  }

  return (
    <HStack gap={5}>{stars}</HStack>
  )
}
